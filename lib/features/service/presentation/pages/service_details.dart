...
class ServiceDetailsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final vm = context.watch<ServicesDetailViewModel>();
    final serviceVm = context.watch<ServicesViewModel>();

    return Scaffold(
      bottomNavigationBar: const Padding(padding: EdgeInsets.all(16), child: HFilledButton(text: 'Book Service')),
      body: SingleChildScrollView(
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _buildMainImageSection(vm, serviceVm, context),
            _buildImagePreviewRow(vm),
            _buildProductHeader(context, serviceVm),
              ...
              child: Column(
                children: [
                  ...
                  SizedBox(
                    ...
                    child: TabBarView(
                      children: [
                        _buildDetailsTab(serviceVm.currentCoreService!),
                        _buildReviewsTab(vm),
                      ],
                    ),
                  ),
            ...
            const SizedBox(height: 80), // For spacing above bottom bar
          ],
        ),
      ),
    );
  ...
  Widget _buildMainImageSection(ServicesDetailViewModel vm, ServicesViewModel serviceVm, BuildContext context) {
    String selectedAction = '';
    final service = serviceVm.currentCoreService;
    return Stack(
      children: [
        CarouselSlider.builder(
          itemCount: service!.images is List ? service.images!.length : vm.images.length,
      ...
      return Column(
        children: [
          ...
          for (final actions in {
            const Icon(Icons.flag_circle_rounded): "Report Service",
            const Icon(Icons.flag): "Report Provider",
            const Icon(Icons.block): "Stop seeing this seller's service",
          }.entries)
            GestureDetector(
                onTap: () {
                  selectedAction = actions.value;
                  if (actions.value == "Report Service") {
                    context.read<ReportViewModel>().initialize(vendor: false);
                    context.goToScreen(SolveitRoutes.reportProductOrVendor);
                  } else if (actions.value == "Report Provider") {
  ...
  Widget _buildImagePreviewRow(ServicesDetailViewModel vm) {
    return Padding(
  ...
  Widget _buildProductHeader(BuildContext context, ServicesViewModel vm) {
    final service = vm.currentCoreService;
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text("₦ ${service.price}", style: context.bodyMedium?.copyWith(fontWeight: FontWeight.bold, color: SolveitColors.primaryColor)),
              Row(
                spacing: 6,
                ...
                children: [
                  ...
                  GestureDetector(
                    ...
                    child: Container(
                      ...
                      child: Text("View service", style: context.bodySmall?.copyWith(fontWeight: FontWeight.bold, fontSize: 10)),
                    ),
                  ),
                ],
              ),
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              _infoChip(Icons.location_on, service.availability, context, isLocation: true),
              _infoChip(Icons.access_time, StringUtils.formatLastActive(service.createdAt), context),
              _infoChip(Icons.remove_red_eye_outlined, '2', context),
            ],
          ),
        ],
      ),
  ...
  Widget _infoChip(IconData icon, String label, BuildContext context, {bool? isLocation = false}) => Container(
        padding: EdgeInsets.only(left: 6.w, right: isLocation! ? 80.w : 20.w, top: 2.h, bottom: 2.h),
        decoration: BoxDecoration(
  ...
  Widget _buildDetailsTab(CoreServiceModel model) => Padding(
  ...
  Widget _buildReviewsTab(ServicesDetailViewModel vm) {
    final ratings = coreServicesViewModel.ratingsResponse?.data ?? [];
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(StringUtils.formatLastActive(review.createdAt), style: context.bodySmall?.copyWith(color: Colors.grey, fontSize: 12)),
                  ],
                ),
              )
            ],
